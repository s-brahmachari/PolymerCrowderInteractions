...
class MiChroM:
    ...
    def randomizePositions(self, strength=0.0001):
        R"""
        Runs automatically to offset the positions if it is an integer (int) variable.
        """
        data = self.getPositions()
        data = data + np.random.randn(*data.shape) * strength
        self.setPositions(data)
    ...
    def addAngles(self, ka=2.0, theta_rad=3.1412):
    ...
    def addSelfAvoidance(self, Ecut=4.0, k_rep=20.0, r0=1.0):
        ...
        repulforceGr.addGlobalParameter('Ecut', Ecut)
        repulforceGr.addGlobalParameter('r_rep', r0)
    ...
    def addGaussianSelfAvoidance(self, Ecut=4.0, r0=1.0):
        R"""
        This adds Soft-core self avoidance between all non-bonded monomers.
        This force is well behaved across all distances (no diverging parts)
        Args:
        ...
        Ecut = Ecut*self.Epsilon
        repul_energy = ("Ecut * exp( - (r / r_rep_gauss)^2)")
        
        self.forceDict["GaussSelfAvoidance"] = self.mm.CustomNonbondedForce(repul_energy)
        repulforceGr = self.forceDict["GaussSelfAvoidance"]
        repulforceGr.addGlobalParameter('Ecut', Ecut)
        repulforceGr.addGlobalParameter('r_rep_gauss', r0)
    ...
    def _initLEBond(self, r0=1.0):
        ...
        if "LEBond" not in list(self.forceDict.keys()):
            force = ("0.5 * k_LE * (r-r_LE)*(r-r_LE)")
            bondforceGr = self.mm.CustomBondForce(force)
            bondforceGr.addPerBondParameter("k_LE")
            bondforceGr.addGlobalParameter("r_LE", r0) 
                
            self.forceDict["LEBond"] = bondforceGr
        

    def addLEBonds(self, loop_extruder_dict, r0=1.0, kfb=30):
        R"""
        Internal function used to add bonds between i and j monomers
        Args:
            i,j (int, required):
        ...
        self._initLEBond(r0=r0)
        for key in loop_extruder_dict:
            i = loop_extruder_dict[key]['left_anchor']
            j = loop_extruder_dict[key]['right_anchor']
            loop_extruder_dict[key]['bond_index'] = self.forceDict["LEBond"].addBond(int(i), int(j), [kfb])
            self.bondsForException.append((int(i), int(j)))
    ...
    def createRandomGas(self, ChromSeq=None, Rc=50.0, isRing=False):    
        ...
        chain = []
        if isRing==True:
            chain.append((0,Nbeads-1,1))
        else:
            chain.append((0,Nbeads-1,0))
        self.setChains(chain)

        return np.random.uniform(-Rc,Rc, size=(Nbeads,3))
